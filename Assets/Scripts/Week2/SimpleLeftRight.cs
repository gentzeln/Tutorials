...
public class SimpleLeftRight : MonoBehaviour
{
    ...
    void Update()
    {
        //Calculate the new X position using a sine wave for smooth movement
        float newX = startPosition.x + Mathf.Sin(Time.time * moveSpeed) * moveAmount;

        // Set the object's position with the new X value while keeping the Y and Z the same!
